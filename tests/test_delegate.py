"""Stdlib unit tests for antigravity-delegate wrapper.

Does not invoke the real `agy` binary or the network.
...
SCRIPT = SKILL_ROOT / "scripts" / "delegate_antigravity.py"
MODULE_NAME = "delegate_antigravity_under_test"
...
class TestFindAgy(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.mod = _load_module()

    def test_honors_agy_bin_env(self) -> None:
        fake = str(Path(tempfile.gettempdir()) / "fake-agy-bin-for-tests.exe")
        previous = os.environ.get("AGY_BIN")
        os.environ["AGY_BIN"] = fake
        try:
            self.assertEqual(self.mod.find_agy(), fake)
        finally:
            if previous is None:
                os.environ.pop("AGY_BIN", None)
            else:
                os.environ["AGY_BIN"] = previous
...
class TestMainCli(unittest.TestCase):
    ...
    def test_timeout_writes_manifest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as output_dir, mock.patch.object(mod, "find_agy", return_value=sys.executable), mock.patch.object(mod, "run_bounded", side_effect=subprocess.TimeoutExpired([], 1, output=b"partial", stderr=b"warning")), mock.patch.object(sys, "argv", ["delegate", "--cwd", str(SKILL_ROOT), "--task", "test", "--output-dir", output_dir]):
            ...
            manifest = json.loads((Path(output_dir) / "result.json").read_text(encoding="utf-8"))
            self.assertEqual((manifest["tool"], manifest["exit_code"], manifest["timed_out"]), ("agy", 124, True))

    def test_nonexistent_cwd_exits_2(self) -> None:
        bogus = str(Path(tempfile.gettempdir()) / "antigravity-delegate-no-such-cwd-xyz")
    ...
    def test_missing_binary_exits_127(self) -> None:
        fake_bin = str(Path(tempfile.gettempdir()) / "missing-agy-xyz123.exe")
        if Path(fake_bin).is_file():
            self.skipTest("unexpected real file at fake bin path")
        env = os.environ.copy()
        env["AGY_BIN"] = fake_bin
        proc = subprocess.run(
        ...
        if proc.returncode not in (127, 126) and "not found" not in combined.lower():
            self.skipTest(
                f"AGY_BIN override appears bypassed (exit {proc.returncode}): {combined[:400]}"
            )
