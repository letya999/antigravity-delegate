...
Headless Antigravity CLI delegate skill by [Artem Letyushev](https://github.com/letya999).

This repository packages a cross-platform agent skill that lets a controlling agent run Antigravity (`agy`) once, non-interactively, capture the result as JSON, and verify the work outside the delegated agent.

## What It Does

- Runs `agy -p` through `scripts/delegate_antigravity.py`
- Captures stdout, stderr, exit code, output files, and the final response
- Supports macOS, Linux, WSL, and Windows
- Uses `AGY_BIN` when the executable is not on `PATH`
- Matches Antigravity print timeout to the wrapper timeout
...
- [dist/antigravity-delegate.zip](dist/antigravity-delegate.zip) is the archive artifact referenced by the index.
